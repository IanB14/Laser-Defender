...
public class PlayerController : MonoBehaviour {
    ...
    void OnTriggerEnter2D(Collider2D laserHit)
    {
        ...
        if (laser)
        {
            ...
            healthKeeper.DecrementHealth(laser.GetDamage());
            laser.Hit();
            if (playerHealth <= 0)
            {
                Die();
    ...
    void Die()
    {
        ...
        LevelManager changeLevel = GameObject.Find("LevelManager").GetComponent<LevelManager>();
        changeLevel.LoadLevel("loseScreen");

        // The above code seems...verbose. I don't know why they don't just instantiate the level manager object.
        //LevelManager changeLevel = new LevelManager();
        //changeLevel.LoadLevel("loseScreen");
        // And if you check the LevelManager class, all it does is call Application.LoadLevel
    }
